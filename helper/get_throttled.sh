#!/bin/bash
#Original from https://github.com/alwye/get_throttled/blob/master/get_throttled.sh
#modifications for sensor log
...
if [ "$CURRENT_HEX" == "0" ] || [ -z $CURRENT_HEX ]; then
  echo -n "No throttling issues detected."
else
  bit_n=0
  for (( i=${#CURRENT_BIN}-1; i>=0; i--)); do
    if [ "${CURRENT_BIN:$i:1}" = "1" ]; then
      echo -n "${ISSUES_MAP[$bit_n]};"
      bit_n=$((bit_n+1))
    fi
  done
fi
